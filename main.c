...
static ffi_fn ffi_load(char *lib_name, char *fn_name) {
    for (int i = 0; i < ffi_cache_len; i++) {
        if (!strcmp(ffi_cache[i].lib_name, lib_name) && !strcmp(ffi_cache[i].fn_name, fn_name)) {
...
static long s_to_long(S *s) {
    if (!s) return 0;
    switch (s->t) {
        case NUM: return (long)s->n;
        case SYM: return (long)(intptr_t)s->s;
        case T_STR: return (long)(intptr_t)s->s;
...
static char *stringify(S *s) {
    ...
    switch (s->t) {
        ...
        case FFI_FN:
            snprintf(tmp, sizeof(tmp), "#<ffi:%s>", s->s ? s->s : "");
            return xstrdup(tmp);
        default:
            return xstrdup("");
    }
...
static void print(S *s, bool in_repl_mode) {
    ...
    switch (s->t) {
        ...
        case FFI_FN: printf("#<ffi:%s>", s->s ? s->s : ""); break;
        case LST:
            putchar('(');
            for (int i = 0; i < s->len; i++) {
                if (i) putchar(' ');
...
static S *read_atom(void) {
    ...
    *wp = '\0';
    ...
    if (isdigit((unsigned char)w[0]) || (w[0] == '-' && isdigit((unsigned char)w[1]))) return n(atof(w));
...
static S *eval(S *e) {
    ...
    if (e->t == NUM || e->t == T_TRUE || e->t == T_FALSE || e->t == T_STR) return e;
    ...
    if (e->l[0]->t == SYM) {
        ...
        if (!strcmp(op, "ffi")) {
            ...
            if (!((lib_cell->t == T_STR) || (lib_cell->t == SYM)) ||
                ...
                fprintf(stderr, "FFI: library and function names must be strings\n");
                return nil();
            }
        ...
        if (!strcmp(op, "strlen")) {
            ...
            S *arg = eval(e->l[1]);
            if (arg->t == T_STR || arg->t == SYM) return n((double)strlen(arg->s ? arg->s : ""));
            return n(0);
        }
        ...
        if (!strcmp(op, "+") || !strcmp(op, "-") || !strcmp(op, "*") || !strcmp(op, "/")) {
            ...
            for (int i = 1; i < e->len; i++) {
                ...
                if (!strcmp(op, "+")) acc += v;
                else if (!strcmp(op, "-")) acc = (i == 1) ? v : acc - v;
                else if (!strcmp(op, "*")) acc *= v;
                else if (!strcmp(op, "/")) acc = (i == 1) ? v : acc / v;
            }
            return n(acc);
        }
    ...
    if (fn->t == LST && fn->len == 2) {
        ...
        for (int i = 0; i < params->len && i + 1 < e->len; i++) {
            if (params->l[i]->t == SYM) env_set(params->l[i]->s, eval(e->l[i + 1]));
        }
        return eval(body);
    }
